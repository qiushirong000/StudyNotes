...
（3）通过值类型状态机和内联方法调用，避免闭包和委托调用，取代C#中async创建的class状态机。
...
缺点：调试难，编辑器下热更调试不直观，很多问题只能在真机上复现。打包更慢，流程更复杂，AOT构建->补元数据->热更DLL->注入。
...
Odin = Odin Inspector（编辑器面板增强）+ Odin Serializer（高性能序列化库，开源），Unity工业化标配插件，零侵入、大幅削减手写Editor代码量。

应用：游戏配置工具、后台调试窗口、批量资源处理工具。
原理：？
优点：提升开发效率，序列化能力碾压原生，零侵入低迁移成本。
缺点：序列化内存占用会略高于极简原生结构，打包体积微小增加。

-------------------------------

## DOTWeen

用途：Unity 高性能补间动画插件，做位移、缩放、透明度、数值、UI、材质、路径等插值动画，替代原生协程 Lerp；
原理：全局单一DoTweenManager管理器，每帧统一获取Time.deltaTime，批量遍历所有激活 Tween 动画，统一计算进度，而非每个动画单独开协程 Update，减少多协程调度开销。动画 Tween实体池化，播放结束不直接 Destroy，回收至池子；新开动画优先取用池内闲置对象，减少频繁 new 堆内存分配。
优点：API 简洁、低 GC、链式调用、大量扩展适配 Transform/UI/Material、自带缓动曲线、支持 UniTask 异步等待；
缺点：极端海量物体动画时仍有微小开销，自定义复杂骨骼动画不如 Animator 原生适配，少数特殊材质属性需手动写访问器。

## Timeline
用途：Unity 官方可视化时间轴剪辑工具，编排多类型复合序列动画（角色动作、镜头、音频、UI、粒子、脚本事件），常用于过场动画、剧情 CG、开场演出；
原理：基于轨道分层存储剪辑片段，Playable 框架驱动逐帧采样插值、调度各类资源与逻辑，挂载 PlayableDirector 播放；
优点：可视化拖拽编辑、多元素协同编排、精准时间轴控速，官方原生无兼容风险；
缺点：代码灵活度弱，轻量小动效冗余，运行时动态修改麻烦；


Timeline 侧重长剧情复合演出、多资源协同剪辑；DOTween 侧重代码驱动轻量化插值动效；一个可视化编排大序列，一个代码快速写小动画。
...
开源完整 Unity 资源管理框架，一站式覆盖Editor 打包、依赖分析、AB 加载、内存管控、全平台热更新、版本灰度回滚，替代原生 Resources 与手写 AB 管理，适配手游、小游戏、PC 网游热更需求。

优点：零冗余打包、自动解依赖、安全内存卸载、多 PlayMode 调试、多 Package 隔离、加密压缩原生支持、兼容 SBP 构建、完善调试分析工具。

缺点：上手有配置学习成本；超大型分布式多包协作需规范约束；相比极简手写 AB 多一层框架调度微小开销；热更逻辑流程多，排查问题链路略长。

### 资源打包流程
（1）YooAsset基于Unity打包系统，底层输出AssetBundle。

（2）按规则分组：按文件、文件夹、标签打包。

（3）自动处理冗余依赖，把共享资源抽到共享包。

（4）构建输出：包名、哈希、版本、Manifest、清单文件。
...
（4）通过LoadAssetAsync加载资源。

（5）内部自动加载依赖包->加载资源->回调返回。

（6）支持引用计数，卸载资源用Release。

### 依赖管理

YooAsset会自动分析依，共享资源会被自动提取为共享包，加载一个资源时引擎会先递归加载其所有依赖包，用引用计数控制包的加载与卸载，确保不重复加载、不漏卸载、不重复依赖。

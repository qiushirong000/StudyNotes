...
条件变量需要锁来保证​​检查条件+进入等待​​的原子性，避免丢失唤醒和竞态条件。
...
### 151. 介绍一下C++11中的std::forward

std::forward是C++11中引入的一个函数模板，用于实现完美转发。它的作用是根据传入的参数，决定将参数以左值引用还是右值引用的方式进行转发。完美转发是为了解决传递参数时的临时对象（右值）被强制转换为左值的问题。std::forward实现完美转发主要用于以下场景：提高模板函数参数传递过程的转发效率。

### 152. C++哪些报错可以被捕获，哪些不可以？

可捕获：标准库异常、自定义异常（需显式throw）。
不可捕获：编译错误（语法错误、类型不匹配、链接错误）、内存违规（空指针解引用、缓冲区溢出）、未定义行为（数组越界、错误的类型转换）、多线程问题。

### 153. C++常见的代码编译错误

语法错误、类型错误、未定义的标识符、链接错误（如函数/变量进行调用时，其只是声明了但是没有被定义）、数组越界、缺少返回值、未初始化的变量、常量修改、作用域问题、空指针访问（解引用空指针会导致运行时错误或崩溃）

### 154. 引用类型是否可以做函数的返回类型，使用的时候有哪些需要注意的地方？
可以，返回引用相当于返回变量的别名，​​不创建副本​​，直接操作原始对象。
​​(1) 优先返回​​全局/静态变量​​、​​成员变量​​或​​传入的引用参数​​。
​​(2) 若无需修改返回值，使用const引用。
​​(3) 避免返回局部变量或临时对象的引用,返回的引用将指向无效内存。


### 155. 单继承和多继承的虚函数的内存结构区别？

（1）在单继承中,派生类对象包含一个虚函数表指针(vptr),指向基类的虚函数表。这个虚函数表存储了基类中所有的虚函数地址。派生类对象的内存布局是:成员变量 + vptr。在多继承中,派生类对象包含多个虚函数表指针(vptr),分别指向各个基类的虚函数表。每个基类在派生类中都有一个单独的虚函数表。派生类对象的内存布局是:基类1成员 + vptr1 + 基类2成员 + vptr2 + 自身成员。
（2）在单继承中,虚函数调用通过单一的vptr即可找到正确的虚函数。在多继承中,虚函数调用需要根据调用者的动态类型,选择相应的vptr来定位虚函数。

### 156. C++的移动语义move函数和完美转发forward函数
移动语义：传统C++拷贝语义存在明显缺陷，对于临时对象（右值）的深拷贝是不必要的性能损耗。move通过右值引用与移动构造函数实现资源高效转移（直接转移所有权）。

完美转发：泛型编程中，我们经常需要把参数“转发”给其他函数，但是传统方式无法保持原始值类别（左值或者右值）。C++11使用了转发引用与std::forward()来实现完美转发。
转发引用（T&&）: 当T是模版类型参数时，T&&可以绑定到任何类型的参数（左值或者右值），根据推导的T类型，将参数转换为原始的值类别。

通过结合移动语义和完美转发，标准库容器的emplace系列函数可以直接在容器内部构造对象，避免不必要的拷贝。